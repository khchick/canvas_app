class DrawingText extends PaintFunction{
    ...
    onMouseDown(coord,event){
        ...
        this.contextReal.font = `${this.fontWeight} ${config.textSize}px ${config.textFont}`;
        this.contextReal.fillStyle = config.strokeCol;
        ...
        $('#textInput').css({"display":"block","transform":"translateY("+coord[1]+"px) translateX("+coord[0]+"px)","font-size":config.textSize+"px","color":config.strokeCol,"font-family":config.textFont,"font-weight":this.fontWeight,"padding":"0"});
    ...
    outputText(ctx){
        ...
        contextReal.fillText(inputText,this.textX[0],this.textY[0]+parseInt(config.textSize));
        // contextReal.stroke();
